...
def render_cad(cad_path: str, output_dir: str, lighting_intensity: float = 0.6, radius: float = 0.3) -> None:
    ...
    # Determine project root and poses path
    current_file = Path(__file__).resolve()
    cnos_root = current_file.parent
    poses_path = cnos_root / 'src' / 'poses' / 'predefined_poses' / 'obj_poses_level2.npy'
    ...
    # Execute the command
    try:
        # Set cwd to cnos_root so that python -m src.poses.pyrender works
        subprocess.run(cmd, check=True, cwd=str(cnos_root))
